...
void fill_grey() {
  ...
  for(i = 0; i < IMG_C; i++) {
    for(j = 0; j < EXTRA_H; j++) {
      for(k = 0; k < NEW_W; k++) {
	fp_data[i*(NEW_W+2)*(NEW_W+2) + (j+1)*(NEW_W+2) + (k+1)] = 0x0080; //1st region
	fp_data[i*(NEW_W+2)*(NEW_W+2) + (j+1)*(NEW_W+2) + (k+1) + ((NEW_W+2)*(EXTRA_H+NEW_H))] = 0x0080; //2nd region
      }
    }
  }
  end = timer_get_count_us(TIMER);
...
void resize_image() {
  ...
  for(k = 0; k < IMG_C; k++) { 					// 3
	...
	//Save new value
	fp_data[k*(NEW_W+2)*(NEW_W+2) + (r+1)*(NEW_W+2) + (c+1) + ((NEW_W+2)*EXTRA_H)] = val_h;
